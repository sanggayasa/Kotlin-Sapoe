...
public class MockLocationDetector {
    ...
    public static boolean checkForAllowMockLocationsApps(Context context, ArrayList<String> whitelist) {
                ...
                if (requestedPermissions != null) {
                    for (int i = 0; i < requestedPermissions.length; i++) {
                        if (requestedPermissions[i].equals("android.permission.ACCESS_MOCK_LOCATION")
                                ...
                                && !applicationInfo.packageName.startsWith("com.google.")
                                && !applicationInfo.packageName.equalsIgnoreCase("com.factory.mmigroup")
                                && !whitelist.contains(applicationInfo.packageName)
                        ) {
                            FirebaseCrashlytics.getInstance().recordException(new RuntimeException("Fake GPS Detected: " + applicationInfo.packageName +" : " + applicationInfo.nonLocalizedLabel));
                            Toast.makeText(context, "Installed "+ applicationInfo.packageName +" : " + applicationInfo.nonLocalizedLabel, Toast.LENGTH_LONG ).show();
                            count++;
                        }
                    }
                }
