...
class CameraBeforeFragment :BaseMvpFragment<CameraBeforePresenter>(), CameraBeforeContract.View {
    override fun gotoNextPage() {

    }
    @Inject
    override lateinit var presenter: CameraBeforePresenter
        ...
        context?.let {
            when (requestCode) {
                CodeIntent.UPLOAD_IMAGE -> {
                    ...
                    if (success) {
                        showLoading()
                        val imageFile = BitmapFactory.decodeFile(ImagePicker.getImagePath(activity!!, ImagePicker.TEMP_IMAGE_NAME))
                        if(imageFile!=null) {
                            dismissLoading()
                            ...
                            listener?.let {
                                it.getCameraBefore("data:image/png;base64,"+
    ...
    interface Listener{
        fun getCameraBefore(imagefile:String)
    }
}
