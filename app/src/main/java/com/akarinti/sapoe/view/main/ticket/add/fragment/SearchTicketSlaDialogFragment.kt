...
import com.akarinti.sapoe.view.main.ticket.adapter.SlaAdapter
import com.sothree.slidinguppanel.SlidingUpPanelLayout
import kotlinx.android.synthetic.main.dialog_route.*
import kotlinx.android.synthetic.main.dialog_route_internal.*


class SearchTicketSlaDialogFragment : BaseDialogFragment() {

    companion object {
        fun newInstance(): SearchTicketSlaDialogFragment = SearchTicketSlaDialogFragment()
    }
    ...
    private fun initView() {
        tvLokasiRute.setText(R.string.pilih_jenis_tiket_t)
        etSearch.setHint(R.string.cari_jenis_tiket_hint)
        activity?.let { a ->
            view?.let { v ->
                KeyboardUtil(a, v)
            }
        ...
        listener?.let {
            val slaAdapter = SlaAdapter(it.getSla())
            slaAdapter.setOnItemClickListener { _, _, position ->
                it.selectedSla(slaAdapter.data[position])
                dismiss()
            }
            rvRoute.apply {
        ...
        slidingUp.addPanelSlideListener(object : SlidingUpPanelLayout.PanelSlideListener {
            ...
            override fun onPanelStateChanged(panel: View?,
                ...
                if (newState == SlidingUpPanelLayout.PanelState.COLLAPSED) {
                    this@SearchTicketSlaDialogFragment.dismiss()
                }
            }
        })
    }
...
//            })
    ...
    override fun initSubscription() {
        ...
        addUiSubscription(RxBus.listen(SlaFetchUpdateEvent::class.java).subscribe {
            listener?.let {
                (rvRoute?.adapter as? SlaAdapter)?.setNewData(it.getSla())
            }
        })
    }

    interface Listener {
        fun getSla(): ArrayList<TicketSla>
        fun selectedSla(selected: TicketSla)
        fun searchSla(query: String)
    }
}